;;; nano-splash.el --- Nice splash screen at startup -*- lexical-binding: t; -*-
  ...
  (let* ((splash-buffer  (get-buffer-create "*splash*"))
    ...
    (if (eq 0 (length (cl-loop for buf in (buffer-list)
        ...
        (with-current-buffer splash-buffer
          ...
          (insert-char ?\n padding-center)
          (insert (propertize "GNU Emacs / N Λ N O" 'face 'nano-strong))
          (center-line)
          (insert "\n")
          (insert (propertize "Emacs made simple" 'face 'nano-faded))
          (center-line)
